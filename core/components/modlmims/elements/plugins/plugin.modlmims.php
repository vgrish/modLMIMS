...
//    $key = md5($resource->id.$url);
    ...
    /** @var modLMIMS $lmims */
    $lmims = $modx->getObject('modLMIMS', array('key' => $key));
    // lmimsRow not exists
    if ( !($lmims instanceof modLMIMS) ) {
      // create new
      $lmims = $modx->newObject('modLMIMS');
      $lmimsDate = $date;
      $lmimsHash = $hash;
    ...
    break;
  case 'OnDocFormSave':
    if (!$modx->getOption('modlmims.refresh_on_page_save')) return;
